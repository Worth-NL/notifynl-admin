...
def test_begin_register_returns_encoded_options(
    ...
    assert relying_party_options["name"] == "NotifyNL"
...
def test_complete_register_handles_library_errors(
    ...
    mocker.patch(
        "app.models.webauthn_credential.WebAuthnCredential.from_registration", side_effect=RegistrationError("error")
    )
...
def test_verify_webauthn_login_signs_user_in_sends_revalidation_email_if_needed(
    ...
    user_details = {"id": platform_admin_user["id"], "email": platform_admin_user["email_address"]}
