...
def test_should_render_two_factor_page(client_request, api_user_active, mocker, redirect_url):
    ...
    assert page.select_one('a:contains("Not received a text message?")')["href"] == url_for(
        "main.check_and_resend_text_code", next=redirect_url
    )
...
def test_should_send_email_and_redirect_to_info_page_if_user_needs_to_revalidate_email(
    ...
    client_request.post(
        ...
        next=f"/services/{SERVICE_ONE_ID}",
        _data={"sms_code": "12345"},
        _expected_redirect=url_for("main.revalidate_email_sent", next=f"/services/{SERVICE_ONE_ID}"),
    )
...
def test_two_factor_sms_post_should_redirect_to_sign_in_if_user_not_in_session(
    ...
    client_request.post("main.two_factor_sms", _data={"sms_code": "12345"}, _expected_redirect=url_for("main.sign_in"))
...
def test_valid_two_factor_email_link_shows_interstitial(
    ...
    token_url = url_for("main.two_factor_email_interstitial", token=encoded_token, **extra_args)
...
def test_two_factor_email_link_used_when_user_already_logged_in(client_request, valid_token):
    client_request.post_url(
        url_for_endpoint_with_token("main.two_factor_email", token=valid_token),
        _expected_redirect=url_for("main.show_accounts_or_dashboard"),
    )
