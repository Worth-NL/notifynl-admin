...
def test_should_404_if_non_sms_template_for_tour_step(client_request, fake_uuid, mocker, template_type, method):
    ...
    getattr(client_request, method)(
        "main.tour_step", service_id=SERVICE_ONE_ID, template_id=fake_uuid, step_index=1, _expected_status=404
    )
...
def test_should_404_for_get_tour_step_0(
    ...
    client_request.get(
        "main.tour_step", service_id=SERVICE_ONE_ID, template_id=fake_uuid, step_index=0, _expected_status=404
    )
...
def test_back_link_from_get_tour_step_points_to_previous_step(
    ...
    page = client_request.get("main.tour_step", service_id=SERVICE_ONE_ID, template_id=fake_uuid, step_index=2)
