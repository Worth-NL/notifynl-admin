...
def test_upload_csv_file_shows_error_banner(
    ...
    mock_set_metadata.assert_called_once_with(
        SERVICE_ONE_ID, fake_uuid, bucket="test-contact-list", original_file_name="invalid.csv"
    )
...
def test_upload_csv_file_sanitises_and_truncates_file_name_in_metadata(
    ...
    mocker.patch("app.models.contact_list.get_csv_metadata", return_value={"original_file_name": filename})
    client_request.post(
        "main.upload_contact_list",
        service_id=SERVICE_ONE_ID,
    ...
    assert len(mock_set_metadata.call_args_list[0][1]["original_file_name"]) < len(filename)
...
def test_upload_csv_shows_ok_page(client_request, mock_get_live_service, fake_uuid, mocker):
    ...
    mocker.patch("app.models.contact_list.get_csv_metadata", return_value={"original_file_name": "good times.xlsx"})
...
def test_save_contact_list(
    ...
    mock_get_metadata = mocker.patch(
        "app.models.contact_list.get_csv_metadata",
        return_value={"row_count": 999, "valid": True, "original_file_name": "example.csv", "template_type": "email"},
    )
    client_request.post(
        "main.save_contact_list",
        service_id=SERVICE_ONE_ID,
...
def test_cant_save_bad_contact_list(
    ...
    mocker.patch(
        "app.models.contact_list.get_csv_metadata",
        return_value={"row_count": 999, "valid": False, "original_file_name": "example.csv", "template_type": "email"},
    )
    client_request.post(
        "main.save_contact_list",
        service_id=SERVICE_ONE_ID,
...
def test_view_contact_list(
    ...
    mocker.patch(
        ...
        return_value=contact_list_json(
            created_at="2020-03-03T12:12:12.000000Z", service_id=SERVICE_ONE_ID, has_jobs=has_jobs
        ),
    )
    mocker.patch(
        "app.models.contact_list.s3download",
        return_value="\n".join(["email address"] + [f"test-{i}@example.com" for i in range(51)]),
    )
    page = client_request.get(
        "main.contact_list",
        service_id=SERVICE_ONE_ID,
