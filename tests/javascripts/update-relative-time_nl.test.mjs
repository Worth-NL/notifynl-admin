...
import UpdateRelativeTime from '../../app/assets/javascripts/esm/update-relative-time_nl.mjs';

describe('UpdateRelativeTime (nl)', () => {
  ...
  describe('on page load', () => {
    ...
    test('updates relative time text for all elements on the page in nl-NL', () => {
      // Changed to 60s and 120s to avoid the "zojuist" thresholds
      document.body.innerHTML = `
      ...
      expect(elements[0].textContent).toBe('1 minuut geleden');
      expect(elements[1].textContent).toBe('2 minuten geleden');
    });

    test('sets a human-readable title attribute text in nl-NL', () => {
      const el = setElementDateTime(0);
      new UpdateRelativeTime(selector);

      const expectedTitle = new Intl.DateTimeFormat('nl-NL', {
        dateStyle: 'long',
        timeStyle: 'short'
  ...
  describe('for dynamically added elements', () => {
    test('picks up elements added to the DOM after the next interval tick', () => {
      ...
      expect(dynamicEl.textContent).toBe('2 minuten geleden');
    ...
    test('automatically updates the relative time text as time passes', () => {
      ...
      new UpdateRelativeTime(selector);
      expect(el.textContent).toBe('zojuist');

      jest.advanceTimersByTime(60000); // Fast forward 1m (now 70s ago)
      expect(el.textContent).toBe('1 minuut geleden');
    });
  ...
  describe('text shows correct units for specific elapsed time threshold', () => {
    const scenarios = [
      { label: 'zojuist (under 30s)', offset: 25000, expected: 'zojuist' },
      { label: 'in de afgelopen minuut (30s to 59s)', offset: 45000, expected: 'in de afgelopen minuut' },
      { label: 'minutes (at 60s boundary)', offset: 60000, expected: '1 minuut geleden' },
      { label: 'hours (at 45m boundary)', offset: 45 * 60000, expected: '1 uur geleden' },
      { label: 'hours (21h)', offset: 21 * 3600000, expected: '21 uur geleden' },
      { label: 'yesterday (at 22h boundary)', offset: 22 * 3600000, expected: 'gisteren' },
      { label: 'days (26 days)', offset: 26 * 86400000, expected: '26 dagen geleden' },
      { label: 'last month', offset: getCalendarOffset('month', 1), expected: 'vorige maand' },
      { label: 'last year', offset: getCalendarOffset('year', 1), expected: 'vorig jaar' },
    ];
