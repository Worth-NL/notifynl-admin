...
def tour_step(service_id, template_id, step_index):
    ...
    try:
        ...
        if all_placeholders_in_session(placeholders):
            return redirect(url_for(".check_tour_notification", service_id=current_service.id, template_id=template_id))
        return redirect(url_for(".tour_step", service_id=current_service.id, template_id=template_id, step_index=1))
    ...
    if form.validate_on_submit():
        ...
        if all_placeholders_in_session(placeholders):
            return redirect(url_for(".check_tour_notification", service_id=current_service.id, template_id=template_id))
        return redirect(
            url_for(".tour_step", service_id=current_service.id, template_id=template_id, step_index=step_index + 1)
        )
