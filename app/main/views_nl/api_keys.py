...
def revoke_api_key(service_id, key_id):
    ...
    if request.method == "GET":
        flash(
            [
                f"Weet u zeker dat u ‘{key.name}’ wilt intrekken?",
                "U kunt deze API-sleutel dan niet meer gebruiken om verbinding te maken met NotifyNL.",
            ],
            "revoke this API key",
        )
        return render_template(
        ...
        flash(f"‘{key.name}’ is ingetrokken", "default_with_tick")
