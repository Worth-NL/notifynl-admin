...
// NL override of update-relative-time.mjs - uses the nl-NL locale instead of en-GB
class UpdateRelativeTime {
  // creating formatters is expensive so we create them once and reuse
  static relativeFormatter = new Intl.RelativeTimeFormat('nl-NL', { numeric: 'auto' });
  static fullDateFormatter = new Intl.DateTimeFormat('nl-NL', { dateStyle: 'long', timeStyle: 'short' });
  ...
  calculateRelativeTimeString(elementTime, currentTime) {
    ...
    // ensure we are at least at 60s to avoid overlapping with "in de afgelopen minuut"
